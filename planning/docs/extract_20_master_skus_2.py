...
# Collect next 20 data rows after header (rows 22-41)
rows = []
for r in range(header_row+21, header_row+41):
...
for row in rows:
    dst_ws.append(row)
dst_wb.save('planning/docs/sku_recipe_upload_sample_2.xlsx')

# Also write to CSV for reference
with open('planning/docs/sku_recipe_upload_sample_2.csv', 'w', newline='', encoding='utf-8') as f:
    writer = csv.writer(f)
