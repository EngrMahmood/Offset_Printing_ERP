    ...
    function renderSearchResults(results, completedMatches) {
        ...
        if (!results.length) {
            ...
            if (completedMatches && completedMatches.length) {
                ...
                jobCardSearchMeta.textContent = `${names} already Completed — no new packing entries needed.`;
                const doneRow = document.createElement('div');
                doneRow.className = 'dispatch-search-result-item';
                doneRow.style.cursor = 'default';
                doneRow.innerHTML = `<div class="result-main">✅ ${names}</div><div class="result-meta">Already Completed — see Packing Records for its history.</div>`;
